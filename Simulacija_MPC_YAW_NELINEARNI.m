...
A_continous_time_YAW = zeros(6,6);
A_continous_time_YAW(1, 1) = -1.0 / Tgm_;
A_continous_time_YAW(2, 2) = -1.0 / Tgm_;
A_continous_time_YAW(3, 3) = -1.0 / Tgm_;
A_continous_time_YAW(4, 4) = -1.0 / Tgm_;
A_continous_time_YAW(5, 6) =1.0 ;
A_continous_time_YAW(6, :)  =[0.513756355684363;
  -0.513756355684363;
   0.513756355684363;
  -0.513756355684363;
                   0;
...
B_continous_time_YAW = zeros(6,4);
B_continous_time_YAW(1, 1) = 1.0 / Tgm_;
B_continous_time_YAW(2, 2) = 1.0 / Tgm_;
B_continous_time_YAW(3, 3) = 1.0 / Tgm_;
B_continous_time_YAW(4, 4) = 1.0 / Tgm_;


kSamplingTime_YAW = 0.105;
A_dis_calc_YAW = expm(kSamplingTime_YAW*A_continous_time_YAW);

count_integral_A = 10000;
integral_exp_A_YAW = 0*A_dis_calc_YAW;

%approximation of integration
for i = 1: count_integral_A
    integral_exp_A_YAW = (integral_exp_A_YAW + expm((A_continous_time_YAW * kSamplingTime_YAW * i / count_integral_A)) ...
        * kSamplingTime_YAW / count_integral_A);
end


B_dis_calc_YAW = integral_exp_A_YAW * B_continous_time_YAW;



%linearni
R_YAW =inv(diag([100 100 100 100]))* diag([1 1 1 1])  * inv(diag([100 100 100 100]));
R_delta_YAW = inv(diag([100 100 100 100]))* diag([0.1  0.1 0.1 0.1])  * inv(diag([100 100 100 100]));
Q_YAW = inv(diag([100 100 100 100 2*pi 1]))* diag([20 20 20 20 300000 100])  * inv(diag([100 100 100 100 2*pi 1]));

%Nelinearni
R_YAW =inv(diag([100 100 100 100]))* diag([1 1 1 1])  * inv(diag([100 100 100 100]));
R_delta_YAW = inv(diag([100 100 100 100]))* diag([0.1  0.1 0.1 0.1])  * inv(diag([100 100 100 100]));
Q_YAW = inv(diag([100 100 100 100 2*pi 1]))* diag([20 20 20 20 30000000 4000000])  * inv(diag([100 100 100 100 2*pi 1]));
