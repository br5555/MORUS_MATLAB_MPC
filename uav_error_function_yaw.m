...
% R_YAW =diag([pop(1), pop(1), pop(1), pop(1)]);
% R_delta_YAW = diag([pop(2),pop(2), pop(2),pop(2)]);
% Q_YAW = diag([pop(3),pop(3),pop(3),pop(3),pop(4), pop(5)]);


R_YAW =diag([pop(1), pop(1), pop(1), pop(1)]);
R_delta_YAW = diag([0 0 0 0]);
Q_YAW = diag([pop(2),pop(2),pop(2),pop(2),pop(3), pop(4)]);
...
warning('off','all');
    ...
    try
        ...
        simOut = sim('MORUS_NELINEARNI_NELINEARNI_YAW',[],options);
...
%         cost4 = sum(abs(masa_1.signals.values))*1e9 +sum(abs(masa_2.signals.values))*1e9
        cost_func =cost_func + 1e12*sum(abs(error_YAW.signals.values))  +sum(abs(preb_GA2.signals.values) )*5e9 + (sim_time- simOut(end))*1e9 ;
