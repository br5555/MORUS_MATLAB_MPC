...
% 
% %PSO with mass included (stednja energije)
...
sim_time = 200.0;
polu = 1.0;
sim_time  = 10+t1+t2+t3+t4+t5+t6+t7+3

%%
%sim MORUS_NELINEARNI_GA



% x = load('backUpPolunelinearniPoremecaj.mat');
% names = fieldnames(x)
% for iname = 1:length(names)
%   x.(['Adam_', names{iname}]) = x.(names{iname});
%   x = rmfield(x, names{iname});
% end
% save('Adam_polunelinearni_poremecaj_workspace.mat', '-struct', 'x');